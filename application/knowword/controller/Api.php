...
namespace app\knowword\controller;


use app\knowword\model\Game;
use app\knowword\model\User;
...
//获取小程序的openid
    ...
    public function begin()
    {
        ...
        //$num=$redis->zscore("AllopenIdKW",$openId); //挑战次数
        $chance_num=$redis->zscore("AllChanceKW",$openId);//机会次数
       // $today_num=$redis->zscore("AllTodayKW",$openId);//今日挑战次数
        if(!is_numeric($chance_num)){
            $chance_num= Db::name("users")->where("openid",$openId)->value("chance_num");
            //$redis->zadd('AllTodayKW', $res['today_num'], $openId);
            //$redis->zadd('AllopenIdKW', $res['chenllage_num'], $openId);
            $redis->zadd('AllChanceKW', $chance_num, $openId);
            $redis->zincrby('AllChanceKW', -1, $openId);
        }else{
            $chance_num=$redis->zscore("AllChanceKW",$openId);
            if($chance_num>0){
                $redis->zincrby('AllChanceKW', -1, $openId);
                //$redis->zincrby('AllopenIdKW', 1, $openId);
                //$redis->zincrby('AllTodayKW', 1, $openId);
            }else{
                $arr=resCode(400,"机会不足",null);
                return $arr;
            }
        ...
        Db::name("game")->where("openid",$openId)->setInc("today_num");
        Db::name("game")->where("openid",$openId)->setInc("chenllage_num");
       // $today_num=$redis->zscore("AllTodayKW",$openId);
        $today_num=$res['today_num'];
    ...
    public function put_cache(){
        $res=Cache::init();
        $redis=$res->handler();
        $chance_num=$redis->zrange('AllChanceKW', 0, -1,true);
        //$num=$redis->zrange('AllopenIdKW', 0, -1,true);
        //$today_num=$redis->zrevrange('AllTodayKW', 0, -1,true);
       ...
       /* foreach($today_num as $k=>$v){
        ...
        //$redis->ZREMRANGEBYRANK ('AllopenIdKW',0,100000);
        $redis->ZREMRANGEBYRANK ('AllChanceKW',0,100000);
        //$redis->ZREMRANGEBYRANK ('AllTodayKW',0,100000);
    ...
    public function check_chance()
    {
        ...
        $redis=$res->handler();
        $openId=input("post.openId");
        $chance=$redis->zscore("AllChanceKW",$openId);
        // print_r($chance);die;
        if(!is_numeric($chance)){
            $chance=Db::name("users")->where("openid",$openId)->value("chance_num");//哎
        }
        if(empty($chance)||$chance===0||$chance==0){
    ...
    public function share_group(){
       ...
       // echo '$openId:'.$openId.'$encryptedData='.$encryptedData.'$iv='.$iv.'$session_key='.$session_key;die;
        ...
        $count=Db::name("share_group")->where("openid",$openId)->where("share_time",'between',[$start_time,$end_time])->count();
        if($count>=10){
            $arr=resCode(400,"今天学习进度完成，休息一下，明天再来哦！",null);
            return  $arr;
        }
        $res=Cache::init();
        $redis=$res->handler();
        ...
        if($errCode==0) {
            ...
            if(!empty($res)&&$res['share_time']<strtotime(date("Y-m-d"))){
                ...
                if(!is_numeric($chance_num)){
                    Db::name("users")->where("openid",$openId)->setInc("chance_num"); //没有缓存
                }else{
                    $redis->zincrby('AllChanceKW', 1, $openId);
                }
                $arr=resCode(200,"ok",null);
                ...
                $arr=resCode(400,"没得到机会，换个群试试！",null);
                return $arr;
            }
        }
    ...
    public function addXcxFormId()
    {
        ...
        $data=cache("knowword_formid");
        if(empty($data)){
            $data[]=$arr;
            cache("knowword_formid",$data);
        }else if(count($data)<5000){
            array_push($data,$arr);
            cache("knowword_formid",$data);
        }else{
            Db::name('xcx_formid')->insertAll($data);
            cache("knowword_formid",null);
        }
    }
