...
class Api extends ApiLogin
{
    public function index()
    {
        ...
        $user_game = $userdao->findGame($user['id']);
        if ($user_game) {
            $data['code'] = 200;
            $data['msg'] = '获取成功';
    ...
    public function get_question()
    {
        ...
        if ($user_game) {
            ...
            $answer = new Answer();
            $question = db('answer')->where("status=1 and layer='{$layer}'")->cache(7200)->find();
            if ($question) {
    ...
    public function check_answer()
    {
        ...
        if ($user_game) {
            ...
            if ($u_answer && $layer) {
                $answer = new Answer();
                $question = db('answer')->where("status=1 and layer='{$layer}'")->find();
                if ($question) {
    ...
    {
        $openid = input("post.openid");
        if (empty($openid)) return ['code' => 400, 'msg' => '请输入openid'];
        $userdao = new User();
        $user = $userdao->findByOpenid($openid);
        $user_id = $user['id'];
        if ($user_id) {
    ...
    public function share_xcc()
    {
        ...
        $user_id = $user['id'];
        $userdao = new User();
        db('user_game')->where("uid='{$user_id}'")->setInc("gold_num", 15);
...
//添加好友
    ...
    {
        $max_layer = db('mx_answer')->where('status=1')->count();
        print_r($max_layer);
    }
}