...
class Answer extends Model
{
    ...
    public function get_one_friend($user_id)
    {
        ...
        $friend_arr2 = db('user_game')->where($where)->field('uid,avatarUrl,nickname,gold_num,idiom_num')->order('idiom_num desc')->limit(8)->select();
    ...
    {
        $ranking_world = cache('mx_world_ranking');
        if (!is_array($ranking_world) || empty($ranking_world)) {
            $world_arr = array();
            ...
            $ranking_world = $world_arr;
        }
        $ranking_detail['data'] = !empty($ranking_world) ? $ranking_world : [];
