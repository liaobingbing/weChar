...
namespace app\points49\controller;


use app\points49\model\Game;
use app\points49\model\User;
...
//获取小程序的openid
    ...
    public function begin()
    {
        ...
        $num=$redis->zscore("AllopenIdPonit",$openId);
        $chance_num=$redis->zscore("AllChancePonit",$openId);
        // print_r($chance_num);die;
        $today_num=$redis->zscore("AllTodayPonit",$openId);
        if(!is_numeric($chance_num)&&!is_numeric($num)&&!is_numeric($today_num)){
            ...
            $redis->zadd('AllTodayPonit', $res['today_num'], $openId);
            $redis->zadd('AllopenIdPonit', $res['chenllage_num'], $openId);
            $redis->zadd('AllChancePonit', $chance_num, $openId);
            $redis->zincrby('AllChancePonit', -1, $openId);
        }else{
            $chance_num=$redis->zscore("AllChancePonit",$openId);
            if($chance_num>0){
                $redis->zincrby('AllChancePonit', -1, $openId);
                $redis->zincrby('AllopenIdPonit', 1, $openId);
                $redis->zincrby('AllTodayPonit', 1, $openId);
            }else{
    ...
    public function put_cache(){
        $res=Cache::init();
        $redis=$res->handler();
        $chance_num=$redis->zrange('AllChancePonit', 0, -1,true);
        $num=$redis->zrange('AllopenIdPonit', 0, -1,true);
        $today_num=$redis->zrevrange('AllTodayPonit', 0, -1,true);
        ...
        $redis->ZREMRANGEBYRANK ('AllopenIdPonit',0,100000);
        $redis->ZREMRANGEBYRANK ('AllChancePonit',0,100000);
        $redis->ZREMRANGEBYRANK ('AllTodayPonit',0,100000);
    ...
    public function check_chance()
    {
        ...
        $redis=$res->handler();
        $openId=input("post.openId");
        $chance=$redis->zscore("AllChancePonit",$openId);
        // print_r($chance);die;
    ...
    public function addXcxFormId()
    {
        ...
        $data=cache("points49_formid");
        if(empty($data)){
            $data[]=$arr;
            cache("points49_formid",$data);
        }else if(count($data)<5000){
            array_push($data,$arr);
            cache("points49_formid",$data);
        }else{
            Db::name('xcx_formid')->insertAll($data);
            cache("points49_formid",null);
        }
    }
