...
namespace app\gsmatch\controller;


use app\gsmatch\model\Game;
use app\gsmatch\model\User;
...
//获取小程序的openid
    ...
    public function begin()
    {
        ...
        $num=$redis->zscore("AllopenIdGsmatch",$openId);
        $chance_num=$redis->zscore("AllChanceGsmatch",$openId);
        // print_r($chance_num);die;
        $today_num=$redis->zscore("AllTodayGsmatch",$openId);
        if(!is_numeric($chance_num)&&!is_numeric($num)&&!is_numeric($today_num)){
            ...
            $redis->zadd('AllTodayGsmatch', $res['today_num'], $openId);
            $redis->zadd('AllopenIdGsmatch', $res['chenllage_num'], $openId);
            $redis->zadd('AllChanceGsmatch', $chance_num, $openId);
            $redis->zincrby('AllChanceGsmatch', -1, $openId);
        }else{
            $chance_num=$redis->zscore("AllChanceGsmatch",$openId);
            if($chance_num>0){
                $redis->zincrby('AllChanceGsmatch', -1, $openId);
                $redis->zincrby('AllopenIdGsmatch', 1, $openId);
                $redis->zincrby('AllTodayGsmatch', 1, $openId);
            }else{
    ...
    public function put_cache(){
        $res=Cache::init();
        $redis=$res->handler();
        $chance_num=$redis->zrange('AllChanceGsmatch', 0, -1,true);
        $num=$redis->zrange('AllopenIdGsmatch', 0, -1,true);
        $today_num=$redis->zrevrange('AllTodayGsmatch', 0, -1,true);
        ...
        $redis->ZREMRANGEBYRANK ('AllopenIdGsmatch',0,100000);
        $redis->ZREMRANGEBYRANK ('AllChanceGsmatch',0,100000);
        $redis->ZREMRANGEBYRANK ('AllTodayGsmatch',0,100000);
    ...
    public function check_chance()
    {
        ...
        $redis=$res->handler();
        $openId=input("post.openId");
        $chance=$redis->zscore("AllChanceGsmatch",$openId);
        // print_r($chance);die;
    ...
    public function addXcxFormId()
    {
        ...
        $data=cache("Gsmatch_formid");
        if(empty($data)){
            $data[]=$arr;
            cache("Gsmatch_formid",$data);
        }else if(count($data)<5000){
            array_push($data,$arr);
            cache("Gsmatch_formid",$data);
        }else{
            Db::name('xcx_formid')->insertAll($data);
            cache("Gsmatch_formid",null);
        }
    }
