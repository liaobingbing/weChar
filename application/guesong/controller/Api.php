...
namespace app\guesong\controller;


use app\guesong\model\Game;
use app\guesong\model\User;
...
//获取小程序的openid
    ...
    public function begin()
    {
        ...
        $num=$redis->zscore("AllopenIdGues",$openId);
        $chance_num=$redis->zscore("AllChanceGues",$openId);
        // print_r($chance_num);die;
        $today_num=$redis->zscore("AllTodayGues",$openId);
        if(!is_numeric($chance_num)&&!is_numeric($num)&&!is_numeric($today_num)){
            ...
            $redis->zadd('AllTodayGues', $res['today_num'], $openId);
            $redis->zadd('AllopenIdGues', $res['chenllage_num'], $openId);
            $redis->zadd('AllChanceGues', $chance_num, $openId);
            $redis->zincrby('AllChanceGues', -1, $openId);
        }else{
            $chance_num=$redis->zscore("AllChanceGues",$openId);
            if($chance_num>0){
                $redis->zincrby('AllChanceGues', -1, $openId);
                $redis->zincrby('AllopenIdGues', 1, $openId);
                $redis->zincrby('AllTodayGues', 1, $openId);
            }else{
    ...
    public function put_cache(){
        $res=Cache::init();
        $redis=$res->handler();
        $chance_num=$redis->zrange('AllChanceGues', 0, -1,true);
        $num=$redis->zrange('AllopenIdGues', 0, -1,true);
        $today_num=$redis->zrevrange('AllTodayGues', 0, -1,true);
        ...
        $redis->ZREMRANGEBYRANK ('AllopenIdGues',0,100000);
        $redis->ZREMRANGEBYRANK ('AllChanceGues',0,100000);
        $redis->ZREMRANGEBYRANK ('AllTodayGues',0,100000);
    ...
    public function check_chance()
    {
        ...
        $redis=$res->handler();
        $openId=input("post.openId");
        $chance=$redis->zscore("AllChanceGues",$openId);
        // print_r($chance);die;
    ...
    public function addXcxFormId()
    {
        ...
        $data=cache("guesong_formid");
        if(empty($data)){
            $data[]=$arr;
            cache("guesong_formid",$data);
        }else if(count($data)<5000){
            array_push($data,$arr);
            cache("guesong_formid",$data);
        }else{
            Db::name('xcx_formid')->insertAll($data);
            cache("guesong_formid",null);
        }
    }
