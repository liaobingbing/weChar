...
namespace app\chinese\controller;


use app\chinese\model\Game;
use app\chinese\model\User;
...
//获取小程序的openid
    ...
    public function index()
    {
        ...
        $redis=$res->handler();
        $openId=input("post.openId");
        $chance=$redis->zscore("AllChanceChinese",$openId);
        // print_r($chance);die;
    ...
    public function begin()
    {
        ...
        $redis=$res->handler();
        $openId=input("post.openId");
        $chance_num=$redis->zscore("AllChanceChinese",$openId);
        if(!is_numeric($chance_num)){
            $chance_num= Db::name("users")->where("openid",$openId)->value("chance_num");
            $redis->zadd('AllChanceChinese', $chance_num, $openId);
            $chance_num=$redis->zscore("AllChanceChinese",$openId);
            if($chance_num>0){
                $redis->zincrby('AllChanceChinese', -1, $openId);
            }else{
    ...
    public function put_cache(){
        $res=Cache::init();
        $redis=$res->handler();
        $chance_num=$redis->zrange('AllChanceChinese', 0, -1,true);

        foreach($chance_num as $k=>$v){
            Db::name("users")->where("openid",$k)->setField("chance_num",$v);
        }
        $redis->ZREMRANGEBYRANK ('AllChanceChinese',0,100000);
    }
    //判断是否有挑战机会
...
//保存关卡
        ...
        if($errCode==0) {
            ...
            if(!empty($res)&&$res['share_time']<strtotime(date("Y-m-d"))){
                ...
                if(!is_numeric($chance_num)){
                    ...
                    $redis->zincrby('AllChanceChinese', 2, $openId);
                }
                $arr=resCode(200,"ok",null);
                return $arr;
            }else if(empty($res)){
    ...
    public function addXcxFormId()
    {
        ...
        $data=cache("chinese_formid");
        if(empty($data)){
            $data[]=$arr;
            cache("chinese_formid",$data);
        }else if(count($data)<5000){
            array_push($data,$arr);
            cache("chinese_formid",$data);
        }else{
            Db::name('xcx_formid')->insertAll($data);
            cache("chinese_formid",null);
        }
    }
