...
class Answer extends Model{
    ...
    public function answer_success($uid,$layer){
        ...
        if($user_game){
            if($layer){
                ...
                $info=db('user_game')->where(array('uid'=>$uid))->update($user_game);
                if($info){
                    $level_arr=db('level')->where('status=1')->cache(86400)->select();
                    $data['up_status']=0;
                    $data['up_layer']='';
    ...
    public function get_one_friend($user_id){
        ...
        if(!$friend_detail){*/
            ...
            $friend_arr2=db('user_game')->where($where)->field('uid,avatar_url,nickname,gold_num,success_num')->order('success_num desc')->select();
    ...
    public function get_world_ranking($user_id){
        $ranking_detail['data']=cache('gd_world_ranking');
    ...
    public function get_help_answer($uid,$layer){
        $info=db('user_help')->where(array('uid'=>$uid,'layer'=>$layer))->field('help_answer,user_avatarUrl')->select();
        if($info){
            $top=0;
            foreach($info as $k=>$v){
                $info[$k]['top']=$top;
    ...
    public function user_share_group($uid,$gid){
        ...
        if($uid&&$gid){
            ...
            if($user_game){
                ...
                $has=db('share_group')->where(array('uid'=>$uid,'openGid'=>$gid))->find();
                if($has){
                    if($has['share_time']<strtotime(date("Y-m-d"),time())){
                        ...
                        $has['share_time']=time();
                        db('share_group')->where("uid",$uid)->update($has);
                        $info['add_status']=1;
