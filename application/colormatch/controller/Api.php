...
namespace app\colormatch\controller;


use app\colormatch\model\Game;
use app\colormatch\model\User;
...
//获取小程序的openid
    ...
    public function get_question()
    {
        ...
        $word=$game->get_answer($res,$layer);
        if($res){
            $arr=resCode(200,"查询成功",array("word"=>$word,"answer"=>$res['option_1'],"next_layer"=>$layer+1));
        }else{
    ...
    public function begin()
    {
        ...
        $redis=$res->handler();
        $openId=input("post.openId");
        $chance_num=$redis->zscore("AllChanceCmatch",$openId);
        if(!is_numeric($chance_num)){
            $chance_num= Db::name("users")->where("openid",$openId)->value("chance_num");
            $redis->zadd('AllChanceCmatch', $chance_num, $openId);
            $chance_num=$redis->zscore("AllChanceCmatch",$openId);
            if($chance_num>0){
                $redis->zincrby('AllChanceCmatch', -1, $openId);
            }else{
    ...
    public function put_cache(){
        $res=Cache::init();
        $redis=$res->handler();
        $chance_num=$redis->zrange('AllChanceCmatch', 0, -1,true);

        foreach($chance_num as $k=>$v){
            Db::name("users")->where("openid",$k)->setField("chance_num",$v);
        }
        $redis->ZREMRANGEBYRANK ('AllChanceCmatch',0,100000);
    }
    //判断是否有挑战机会
...
//保存关卡
    public function put_layer()
    {
        ...
        if($layer>$res){
            ...
            $arr=resCode(400,"继续加油啊,没有超过最高关",$res);
            return $arr;
        }
    }
    //排行榜 依照获取的现金数
       ...
       // echo '$openId:'.$openId.'$encryptedData='.$encryptedData.'$iv='.$iv.'$session_key='.$session_key;die;
        ...
        if($errCode==0) {
            ...
            if(!empty($res)&&$res['share_time']<strtotime(date("Y-m-d"))){
                Db::name("share_group")->where("openGid",$openGid)->where("openid",$openId)->setField("share_time",time());
                if(!is_numeric($chance_num)){
                    Db::name("users")->where("openid",$openId)->setInc("chance_num",2);
                }else{
                    $redis->zincrby('AllChanceCmatch', 2, $openId);
                }
                $arr=resCode(200,"ok",null);
                return $arr;
            }else if(empty($res)){
    ...
    public function addXcxFormId()
    {
        ...
        $data=cache("colormatch_formid");
        if(empty($data)){
            $data[]=$arr;
            cache("colormatch_formid",$data);
        }else if(count($data)<5000){
            array_push($data,$arr);
            cache("colormatch_formid",$data);
        }else{
            Db::name('xcx_formid')->insertAll($data);
            cache("colormatch_formid",null);
        }
    }
