...
class AnswerModel extends Model{
    ...
    public function answer_success($uid,$layer){
        ...
        if($user_game){
            if($layer){
                ...
                if($info){
                    $level_arr=M('level')->where('status=1')->select();
                    $data['up_status']=0;
                    $data['up_layer']='';
    ...
    public function get_world_ranking($user_id){
        $ranking_detail['data']=S('mx_world_ranking');
