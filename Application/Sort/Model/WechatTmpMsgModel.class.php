...
class WechatTmpMsgModel extends Model {
	private $appid;
	private $appsecret;
	...
	public function xcx_get_form_id($open_id) {
		...
		$form_id = M('xcx_formid')->where(array(
			'open_id' => $open_id,
		))
			->order('add_time ASC')
			->getField('form_id');
	...
	public function xcx_delete_form_id() {
		M('xcx_formid')->where(array(
			'add_time' => array('LT', time() - 7 * 24 * 3600 - 30),
	...
	public function xcxSend($data) {
		...
		if ($r['errcode'] == 0) {
			...
			if ($form_id = M('xcx_formid')->where(array('form_id' => $data['form_id'], 'open_id' => $data['touser']))->getField('id')) {
				M('xcx_formid')->where(array('id' => $form_id))->delete();
			}
		}
