...
class Component
{
    ...
    {
        file_put_contents(RUNTIME_PATH.'Logs/Component/'.date('Y-m-d').'log.txt', date('Y-m-d H:i:s') . PHP_EOL, FILE_APPEND);
        if (is_string($data)) {
            file_put_contents(RUNTIME_PATH.'Logs/Component/'.date('Y-m-d').'log.txt', $data . PHP_EOL, FILE_APPEND);
        } else {
            file_put_contents(RUNTIME_PATH.'Logs/Component/'.date('Y-m-d').'log.txt', var_export($data, true), FILE_APPEND);
        }
    }
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @param string $customer_account 转发到指定客服帐号：test1@test
     */
    public function transfer_customer_service_default()
    {
        $msg = array(
            'ToUserName' => $this->getRevFrom(),
            'FromUserName' => $this->getRevTo(),
